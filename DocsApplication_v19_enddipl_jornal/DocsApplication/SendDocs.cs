...
namespace DocsApplication
{
    public partial class SendDocs : MetroFramework.Forms.MetroForm
    {
        ...
        private void metroButton1_Click(object sender, EventArgs e)
        {
            ...
            string id_doc = metroTextBox9.Text;
                ...
                if (key != "")
                {
                    if (match.Success)
                    {
                        try
                        {
                            ...
                            connection.Open();
                            SqlCommand jr = new SqlCommand("INSERT INTO Журнал(Id_пользователь, Id_событие, Дата, Время, Id_документ, Почтовый_адрес) VALUES(@id_p, @id_sob, @dat, @time, @id_doc, @adrs)", connection);
